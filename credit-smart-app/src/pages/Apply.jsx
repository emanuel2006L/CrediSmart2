...
export default function Apply() {
  ...
  // validaciones simples en tiempo real
  useEffect(() => {
    const e = {};
  ...
  function handleSubmit(e) {
    ...
    // guardar solicitud en memoria (array)
        ...
        <div className="solicitud-card">
          ...
          <form className="solicitud-form" onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              ...
              <select id="documento" value={form.docType} onChange={(e) => update("docType", e.target.value)}>
            ...
            <div className="form-group">
              ...
              <select id="tipoCredito" value={form.creditId} onChange={(e) => update("creditId", e.target.value)}>
            ...
            <div className="form-group">
              ...
              <input id="monto" type="number" value={form.amount} onChange={(e) => update("amount", e.target.value)} placeholder="Ej: 20000000" />
            </div>
            {errors.amount && <p style={{ color: "crimson" }}>{errors.amount}</p>}

            <div className="form-group">
              <label htmlFor="plazo">Plazo en meses</label>
              <input id="plazo" type="number" value={form.term} onChange={(e) => update("term", e.target.value)} placeholder="Ej: 24" />
            </div>
          ...
          {requests.length > 0 && (
            <>
              ...
              <ul>
                {requests.map(r => (
                  <li key={r.id}>
                    {r.name} - {creditsData.find(c => String(c.id) === String(r.creditId))?.name || "—"} - Monto: ${formatCurrency(r.amount)} - Cuota: ${formatCurrency(r.monthly)}
                  </li>
                ))}
              </ul>
            </>
